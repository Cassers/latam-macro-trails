<script>
    ...
    function draw() {
        ...
        y.ticks(5).forEach(t => {
            ...
            svg.append('text').attr('class', 'axis-text').attr('x', -8).attr('y', y(t) + 3).attr('text-anchor', 'end').text(t + '%');
        });
        ...
        countries.filter(c => activeList.includes(c)).forEach(c => {
            ...
            vals.forEach(d => {
                g.append('circle').attr('cx', x(d.year)).attr('cy', y(d[dataKey])).attr('r', 2.5).attr('fill', color).attr('cursor', 'pointer')
                    .on('mouseover', e => showTip(`<b style="color:${color}">${c} · ${d.year}</b><br>${label}: ${d[dataKey]}%`, e))
    ...
    $effect(() => { activeList; theme; dataKey; draw(); });
</script>
